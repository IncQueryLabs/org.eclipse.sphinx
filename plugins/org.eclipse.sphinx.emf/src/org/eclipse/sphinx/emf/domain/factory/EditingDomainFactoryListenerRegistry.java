...
public class EditingDomainFactoryListenerRegistry {
	...
	/**
	 * The registered {@linkplain ITransactionalEditingDomainFactoryListener editing domain factory listener}s.
	 */
	private Map<IMetaModelDescriptor, Map<String, ITransactionalEditingDomainFactoryListener>> fEditingDomainFactoryListeners = new HashMap<IMetaModelDescriptor, Map<String, ITransactionalEditingDomainFactoryListener>>();
	...
	private void readContributedEditingDomainFactoryListeners() {
		for (IConfigurationElement cfgElement : Platform.getExtensionRegistry().getConfigurationElementsFor(EXTP_EDITING_DOMAIN_FACTORY_LISTENERS)) {
			try {
				if (NODE_LISTENER.equals(cfgElement.getName())) {
					...
					// Get instance of contributed editing domain factory listener
					ITransactionalEditingDomainFactoryListener listener = (ITransactionalEditingDomainFactoryListener) cfgElement
					...
					for (IConfigurationElement applicableFor : cfgElement.getChildren(NODE_APPLICABLE_FOR)) {
						...
						addListener(mmDescIdPattern, listenerId, listener, overriddenListenerId);
					}
				}
			} catch (Exception ex) {
		...
		for (String overriddenListenerId : fOutstandingOverrides.keySet()) {
			for (IMetaModelDescriptor mmDescriptor : fOutstandingOverrides.get(overriddenListenerId)) {
				Map<String, ITransactionalEditingDomainFactoryListener> listenersForMetaModel = fEditingDomainFactoryListeners.get(mmDescriptor);
	...
	public Collection<ITransactionalEditingDomainFactoryListener> getListeners(IMetaModelDescriptor mmDescriptor) {
		...
		// descriptors
		HashSet<ITransactionalEditingDomainFactoryListener> listeners = new HashSet<ITransactionalEditingDomainFactoryListener>();
		for (IMetaModelDescriptor registeredMMDescriptor : fEditingDomainFactoryListeners.keySet()) {
			if (registeredMMDescriptor.getClass().isInstance(mmDescriptor)) {
				Map<String, ITransactionalEditingDomainFactoryListener> listenersForMetaModel = fEditingDomainFactoryListeners
						.get(registeredMMDescriptor);
				if (listenersForMetaModel != null) {
					listeners.addAll(listenersForMetaModel.values());
				}
			}
		}

		// Retrieve editing domain factory listeners registered upon static any meta-model descriptor
		Map<String, ITransactionalEditingDomainFactoryListener> listenersForAnyMetaModel = fEditingDomainFactoryListeners
				.get(MetaModelDescriptorRegistry.ANY_MM);
		if (listenersForAnyMetaModel != null) {
			listeners.addAll(listenersForAnyMetaModel.values());
		}
		return listeners;
	}

	/**
	 * Adds the specified {@linkplain ITransactionalEditingDomainFactoryListener factory listener} into the map of
	...
	public void addListener(IMetaModelDescriptor mmDescriptor, String listenerId, ITransactionalEditingDomainFactoryListener listener,
		...
		if (mmDescriptor != null && listener != null) {
			...
			Map<String, ITransactionalEditingDomainFactoryListener> listenersForMetaModel = fEditingDomainFactoryListeners.get(mmDescriptor);
			if (listenersForMetaModel == null) {
				listenersForMetaModel = new HashMap<String, ITransactionalEditingDomainFactoryListener>();
	...
	public void removeListener(ITransactionalEditingDomainFactoryListener listener) {
		if (listener != null) {
			Iterator<Map<String, ITransactionalEditingDomainFactoryListener>> iter1;
			for (iter1 = fEditingDomainFactoryListeners.values().iterator(); iter1.hasNext();) {
				Map<String, ITransactionalEditingDomainFactoryListener> listenersForMetaModel = iter1.next();
				Iterator<ITransactionalEditingDomainFactoryListener> iter2;
				for (iter2 = listenersForMetaModel.values().iterator(); iter2.hasNext();) {
					ITransactionalEditingDomainFactoryListener listenerForMetaModel = iter2.next();
